                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <form action="{{ route('resources.users.index') }}" method="GET">
                                <input type="text" id="user-search" name="search" data-kt-user-table-filter="search"
                                    class="form-control form-control-solid w-250px ps-13" placeholder="Search user"  value="{{ request('search') }}" />
                            </form>
                        </div>
                        <!--end::Search-->
                    </div>
                                        ...
                                        @if ($user->roles->count() > 0)
                                            <div class="mt-1">
                                                @foreach ($user->roles as $role)
                                                    <span class="badge badge-light-success fw-bold">{{ $role->name }}</span>
                                                @endforeach
                                            </div>
                                        @endif
                                    </td>
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <form id="delete-form-{{ $user->id }}"
                                                        action="{{ route('resources.users.destroy', ['user' => $user->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
